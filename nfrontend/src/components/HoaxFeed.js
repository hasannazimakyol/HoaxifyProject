...
const HoaxFeed = () => {
    ...
    useEffect(() => {
        ...
        return function cleanup() { //react eğer tekrar bu useEffect'i kullanırsa bir önceki kalan return fonksiyonu çalıştırıp işlemine tekrardan devam ediyor
    ...
    if (content.length === 0) {
        return <div className='alert alert-secondary text-center'>{initialHoaxLoadProgress ? <Spinner /> : t('ThereAreNoHoaxes')}</div>
    }

    return (
        <div>
            {newHoaxCount > 0 &&
                // (<div className='alert alert-secondary text-center mb-1'>{t('ThereAreNewHoaxes')}</div>)
                (<div className='alert alert-secondary text-center mb-1'
                    ...
                    {loadNewHoaxesProgress ? <Spinner /> : t('ThereAreNewHoaxes')}
                </div>)
            }
            {content.map(hoax => {
            ...
            {!last &&
                <div className='alert alert-secondary text-center'
                    ...
                    {loadOldHoaxesProgress ? <Spinner /> : t('LoadOldHoaxes')}
                </div>}
        </div>
    );
};
